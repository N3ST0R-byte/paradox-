...
async def _mute_member(member, muterole, audit_reason=None):
    """
    Mute a given member.
    """
...
async def _unmute_member(member, muterole, audit_reason=None):
    """
...
class MuteAction(_MuteTypeAction):
    ...
    async def action(self):
        ...
        results = await asyncio.gather(
            *(_mute_member(target, self.mute_role, audit_reason=audit_reason) for target in self.targets)
        )
...
class UnMuteAction(_MuteTypeAction):
    ...
    async def action(self):
        ...
        results = await asyncio.gather(
            *(_unmute_member(target, self.mute_role, audit_reason=audit_reason) for target in self.targets)
        )
        member_results = dict(zip(self.targets, results))
        successful = [member.id for member, result in member_results.items() if result is ActionState.SUCCESS]
