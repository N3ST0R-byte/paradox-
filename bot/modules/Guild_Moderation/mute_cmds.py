...
async def cmd_mute(ctx, flags):
    ...
    if muted:
        if len(muted) == 1:
            ...
            reports.append("Muted: `{}`.".format('`, `'.join(muted)))
    if failed:
        if len(failed) == 1:
            reports.append("Failed to mute `{}`.".format(failed[0]))
        else:
            reports.append("Failed to mute: `{}`.".format('`, `'.join(failed)))
...
async def cmd_unmute(ctx, flags):
    ...
    if muted:
        if len(muted) == 1:
            ...
            reports.append("UnMuted: `{}`.".format('`, `'.join(muted)))
    if failed:
        if len(failed) == 1:
            reports.append("Failed to unmute `{}`.".format(failed[0]))
        else:
            reports.append("Failed to unmute: `{}`.".format('`, `'.join(failed)))
