...
async def tex_listener(ctx):
    ...
    if ctx.server:
        await ctx.bot.log("Received the following listening tex message from \"{ctx.author.name}\"(id: {ctx.authid}) in server \"{ctx.server.name}\":\n{ctx.cntnt}".format(ctx=ctx), chid=ctx.ch.id)
    else:
        await ctx.bot.log("Received the following listening tex message from \"{ctx.author.name}\"(id: {ctx.authid}) in DMS:\n{ctx.cntnt}".format(ctx=ctx), chid=ctx.ch.id)
