...
async def cmd_prune(ctx):
    ...
    if not ctx.flags["r"]:
        reason = "None, forced prune" if ctx.flags["force"] else await ctx.request_reason(action="purge")
        if reason is None:
            return
    else:
        reason = ctx.flags["r"]
    ...
    async for message in ctx.bot.logs_from(ctx.ch, limit=1000):
        ...
        to_delete = True
        to_delete = to_delete and (not (ctx.flags["bot"] or ctx.flags["bots"]) or message.author.bot)
        to_delete = to_delete and (not ctx.flags["user"] or not message.author.bot)
        to_delete = to_delete and (not ctx.flags["embed"] or message.embeds)
        to_delete = to_delete and (not ctx.flags["file"] or message.attachments)
        to_delete = to_delete and (not ctx.flags["from"] or message.author == user)
        to_delete = to_delete and (not ctx.flags["me"] or message.author == ctx.me)
        if to_delete:
            i += 1
    ...
    if not abort:
        ...
        try:
            if len(message_list) == 1:
                ...
                await ctx.bot.purge_from(ctx.ch, limit=1000, check=lambda msg: msg.id in msgids)
        except discord.Forbidden:
